...
def fake_urls?(status)
  if status
    ...
    url_shorten_with_key = "https://www.googleapis.com/urlshortener/v1/url?key=Abc123?key=Abc123"
    stub_request(:post, url_shorten_with_key)
      .with(body: params, headers: headers)
      .to_return(load_fixture('shorten.json'))
    ...
    stub_request(:post, url_login)
      .with(body: 'accountType=HOSTED_OR_GOOGLE&service=urlshortener&source=gem-googl-ruby&Email=my_user%40gmail.com&Passwd=my_valid_password',
            headers: headers)
      .to_return(load_fixture('client_login_valid.json'))

    # ClientLogin invalid
    stub_request(:post, url_login)
      .with(body: 'accountType=HOSTED_OR_GOOGLE&service=urlshortener&source=gem-googl-ruby&Email=my_invalid_gmail&Passwd=my_invalid_passwod',
